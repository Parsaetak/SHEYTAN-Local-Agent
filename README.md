...
* **P0 — strict engine-variant correctness** — the variant surface no longer normalizes garbage into CPU: a strict parser (`ParseAssetVariant`) accepts exactly `cpu`/`vulkan` (plus the documented aliases) after controlled normalization and rejects everything else, so `POST /api/engine/provision {"variant":"banana"}` answers a deterministic 400 — invalid input can NEVER become CPU provisioning (the v1.6.1 defect, reproduced then fixed). The lenient reader survives only for legacy manifest reads (a v1.6.0 manifest without the field still reads back as `cpu`). The release resolver is now VARIANT-AWARE: it selects the newest tag whose release payload ACTUALLY CONTAINS the requested variant asset (exact asset-name match + reachability verification) instead of reusing the CPU-oriented `LatestTag()` and hoping the Vulkan URL exists; a release that ships CPU only is skipped, never returned for a Vulkan request, and the current engine tag is preserved when it still serves the variant. The support matrix is ARCHITECTURE-AWARE: Windows x64 advertises CPU+Vulkan (upstream `b11191` serves `llama-b11191-bin-win-vulkan-x64.zip`), Windows ARM64 does NOT advertise Vulkan (no `win-vulkan-arm64` asset in the release — verified), and support is never inferred from the OS alone. CI no longer hard-codes `$tag = "b10642"`: the workflow runs the same authoritative Go resolver gate (`SHEYTAN_CI_VARIANT_GATE=1`) that verifies exact variant asset resolution, exact asset reachability, the support matrix, unsupported-variant refusal and no silent CPU fallback — with the honest note that HTTP reachability is not runtime execution.
* **P0 — GGUF import concurrency/integrity** — the `Stat → choose destination → Rename` race is closed with an exclusive per-models-directory lock (in-process mutex + cross-process advisory file lock: `flock`/`LockFileEx`, both kernel-released on process death). Two concurrent imports of the same filename can no longer overwrite or steal each other's destination: different bytes get collision-safe distinct files (`model.gguf` + `model-1.gguf`), identical bytes yield exactly ONE duplicate result and ONE file, failed imports leave no partial `.tmp`, and the source is still copied, never modified. The streaming/atomic design is unchanged (no whole-model RAM loads). Proven with `go test -race` and cross-process lock tests.
* **P1 — Vulkan runtime truth in the transaction** — the provisioning transaction is now `stop → staged install → startup/health → RUNTIME BACKEND VERIFICATION → commit`: before committing a Vulkan package, the engine's OWN `--list-devices` enumeration (the same authority the accelerator surface uses) must run, and the outcome is recorded honestly — Vulkan device(s) enumerated (confirmed), no Vulkan device on this machine (package committed, GPU offload honestly reported unavailable, CPU fallback still serves), or enumeration unsupported by the build (identity attributed to the manifest only). A verification that cannot EXECUTE rolls the last-known-good package back. A filename, a manifest field or an on-disk `ggml-vulkan.dll` is never accepted as runtime evidence.
* **P1 — sampling contract audit closure** — every launch argument `buildArgsWithCaps` and related paths emit was audited against the llama.cpp parser contract. The remaining gap — the raw user extra arguments (`llamaExtraArgs`) passed through verbatim — is closed: a malformed numeric argument (`--temp abc`, `--top-k 1.5`, `--min-p=2`) now fails deterministically BEFORE process spawn with zero engine spawns, never enters the compatibility ladder, and never causes an accelerator downgrade for a config typo; valid values (including unknown flags — the engine parser stays their authority) pass through unchanged. Repaired sampling values are PERSISTED atomically back into the config file (only the fields that were invalid IN the file; environment-introduced corruption is never baked in), so one repair is enough. `maxTokens` joins the deterministic contract (-1 = unlimited, otherwise a non-negative count).
* **P1 — Settings surface for engine-variant provisioning (API-only no longer)** — Settings → Performance now carries a real "Engine backend" card: installed variant + platform from `GET /api/engine/provision`, provisioning actions through the full transaction (stop → staged install → restart → health/backend verification → UI state), honest hints that GPU claims stay evidence-gated, and a 10-minute request timeout matching the transaction's real duration. Unit coverage: `src/engine-backend.test.ts`.
* **Corrected upstream/Linux facts** — llama.cpp HAS NOT stopped publishing Linux prebuilts: release `b11191` publishes `llama-b11191-bin-ubuntu-x64.tar.gz` and `llama-b11191-bin-ubuntu-vulkan-x64.tar.gz` (plus arm64 variants). They are `.tar.gz` archives, which the zip-based transactional installer does not consume — the honest documented limitation (ROADMAP NEXT: tar.gz installer support), replacing the stale "Linux binaries removed" claim.
...
| Version | Headline |
|---|---|
| v1.6.2 | Strict variant parsing + variant-aware release resolution + architecture-aware support matrix (no silent CPU fallback anywhere), GGUF import race closed (per-directory exclusive lock, -race-verified), runtime backend verification inside the provisioning transaction, extra-args sampling gate closure + persisted repairs, real Settings engine-backend surface, corrected upstream/Linux facts |
...
The engine binary downloads automatically when the machine is online; drop a prebuilt `llama-server(.exe)` into `bin/` for offline installs. On LINUX, upstream llama.cpp still publishes prebuilt packages but as `.tar.gz` archives (verified at release `b11191`: `llama-b11191-bin-ubuntu-x64.tar.gz` and `llama-b11191-bin-ubuntu-vulkan-x64.tar.gz`) — the transactional installer currently consumes `.zip` packages only, so the automatic download cannot succeed on Linux; build `llama-server` from source and set `llamaBinPath`, or select the native engine (`engineBackend: "native"`). Adding `.tar.gz` support to the installer is tracked in ROADMAP.md (NEXT).

**Models** are imported first-class: `Import GGUF…` in the Model Picker (or `POST /api/models/import`) validates the GGUF header, streams the file into `models/` atomically (duplicate-safe, source untouched — and concurrency-safe since v1.6.2: simultaneous imports of the same filename can never overwrite each other) and selects it through the normal selection flow. Placing a `.gguf` file into `models/` by hand still works. On Windows x64 the engine package carries a recorded backend variant (`cpu` or `vulkan`) — Settings → Performance → "Engine backend" (or `POST /api/engine/provision {"variant":"vulkan"}`) swaps in the real Vulkan engine package transactionally; an invalid variant is a deterministic 400 (never a silent CPU install), an explicitly unservable variant fails loudly, and the transaction verifies the runtime backend (engine device enumeration) before committing — a failure rolls back to the last-known-good package. Windows ARM64 does not offer Vulkan (upstream publishes no `win-vulkan-arm64` asset — verified at `b11191`).
