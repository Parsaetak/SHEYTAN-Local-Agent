...
| **Version-only release identity** | GitHub Release title = plain `APP_VERSION` (`1.3.3`); the release job verifies tag == `v${APP_VERSION}` AND title == version after publication; the workflow contract structurally bans the product/tag-derived title, codename fragments (`Zeta`) and `APP_VERSION_FULL`; canonical version 1.3.3 synced across package.json → config.go / build/config.yml / SIGNATURE | IMPLEMENTED, TESTED (28/28 release suite) |
