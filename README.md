...
| Area | Change | Status |
|---|---|---|
| **Version-only identity** | `package.json` is the single source of truth; `release-version.mjs` validates plain semver, synchronizes `config.go` / `build/config.yml` / `SIGNATURE` and emits `APP_VERSION` (`APP_VERSION_FULL` compatibility alias only). No codename dimension exists anywhere | IMPLEMENTED, TESTED |
