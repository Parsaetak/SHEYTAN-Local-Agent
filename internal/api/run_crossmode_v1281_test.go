...
func TestRegenerateKeepsHistoryReferences(t *testing.T) {
        ...
        if reqs := cap.snapshot(); len(reqs) == 0 || !strings.Contains(flattenRequestMessages(reqs[0]), "[HISTORY REFERENCE") {
        ...
        if !strings.Contains(regenPrompt, "[HISTORY REFERENCE") ||
...
func TestSessionContextConcurrentMutationsSurvive(t *testing.T) {
        ...
        go func() {
                ...
                for i := 0; i < rounds; i++ {
                        payload := fmt.Sprintf(
                                `{"historyRefs":[{"sessionId":"agent-src-%d","mode":"agent"}]}`,
                                i%3, // three distinct sources, re-attached repeatedly
                        )
        ...
        for _, ref := range final.Context.HistoryRefs {
                if strings.HasPrefix(ref.SessionID, "agent-src-") {
                        found = true
                }
        }
        if !found {
