...
func TestLateAttachSocketReceivesRunOutcome(t *testing.T) {
	...
	if frame["type"] != "idle" {
		t.Fatalf("first frame type = %v, want idle (run already finished)", frame["type"])
	}
...
func TestActivitiesCarryRunIdAndDoneAttachesMidRun(t *testing.T) {
	...
	// idle sentinel with lastRun is the FIRST frame (also correct). Either
	// way, the loop must terminate on idle-with-evidence or done.
	for i := 0; i < 64; i++ {
		frame := readFrameWithDeadline(t, conn)
		frames++
