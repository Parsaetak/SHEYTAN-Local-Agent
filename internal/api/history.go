...
// run with engineering evidence. Returns the written path ("" = not
// written). Best-effort: the handoff is a durable aid, never a run
// failure; failures are logged and surfaced as an activity caption.
func (s *Server) writeAgentHandoff(sess *sessions.Session, task *agent.TaskState, outcome string) string {
	if sess == nil || task == nil || !task.HasEngineeringEvidence() {
		return ""
	}
