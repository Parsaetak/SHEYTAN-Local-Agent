...
// run 34703102794 (Linux): the host's generate lane used to keep
// opts.prompt pointing at op_generate's stack-local std::string, so a
// loaded runner could reuse the dead stack slot before the lane read it
// and the corrupted prompt silently dropped the BPE merges ("hello"
// encoded as 7 tokens instead of 5, the context test then rejecting a
// fitting request). The lane now owns the prompt. This test fires a
...
// generation lanes. Before the fix, lanes stayed in the host's list for
// its whole lifetime, so after 16 TOTAL generations the host rejected
