...
func (o *Orchestrator) RunDetailed(
	...
	// Model-aware effective context: min(configured, GGUF model limit,
	// engine limit) — a small model is never handed a window larger
	// than it actually has.
	effCtx := o.resolveEffectiveContext(cfg)
	...
	for iter := 0; iter < maxIter; iter++ {
		...
		if elidedTools := compactToolResults(messages, plan.PromptCeiling()); elidedTools > 0 {
			plan.AddAdjustment(fmt.Sprintf(
...
// resolveEffectiveContext computes the model-aware context decision for the
// configured model: min(configured, GGUF model limit, engine-reported
// limit). Remote providers keep their configured value (the remote engine
// owns its window).
func (o *Orchestrator) resolveEffectiveContext(cfg *config.Config) llm.EffectiveContext {
	engineLimit := 0
	if o.ctxLimits != nil {
	...
	if cfg.IsRemote() {
		return llm.ResolveEffectiveContext(cfg, nil, engineLimit)
	}
	...
	return llm.ResolveEffectiveContext(cfg, caps, engineLimit)
}
...
// re-run hint. Returns the number of tool results touched.
func compactToolResults(messages []llm.Message, ceiling int) int {
	if ceiling <= 0 {
		return 0
	}
	...
	for i := 0; i < len(messages) && total > ceiling; i++ {
		...
		messages[i].Content = fmt.Sprintf(
			...
			old)
		total -= old - chunking.EstimateMessagesTokens([]llm.Message{messages[i]})
		elided++
	}
	...
	if total > ceiling && lastTool >= 0 &&
		...
		total -= old - chunking.EstimateMessagesTokens([]llm.Message{messages[lastTool]})
		elided++
	}
