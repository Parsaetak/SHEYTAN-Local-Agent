...
func TestThinkingControlThinkingEnablesNudgeAndDepth(t *testing.T) {
	...
	if taskclassify.TierRank(res.Tier) < taskclassify.TierRank(taskclassify.TierStandard) {
		t.Fatalf("thinking control tier = %q, want >= STANDARD", res.Tier)
	}
...
func TestContextEscalationOnMissingFileEvidence(t *testing.T) {
	...
	if res.Tier != taskclassify.TierFast {
		t.Fatalf("starting tier = %q, want FAST (trivial task)", res.Tier)
	}

	if res.FinalTier != taskclassify.TierStandard {
		t.Fatalf("final tier = %q, want STANDARD after MissingFileContext escalation", res.FinalTier)
	}
	...
	if res.Escalations[0].From != taskclassify.TierFast || res.Escalations[0].To != taskclassify.TierStandard {
		t.Fatalf("escalation move = %s → %s, want FAST → STANDARD",
	...
	if !strings.Contains(escalations[0].Caption, "FAST") || !strings.Contains(escalations[0].Caption, "STANDARD") {
