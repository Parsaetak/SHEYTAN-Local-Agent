...
//     mechanism replaces.
//   - SHELL COMMAND STRINGS ("sh -c", "cmd /c", …) are banned except in
//     the explicitly allowlisted files where a test feeds a command
//     string THROUGH the production machinery that owns shell execution
//     (the Coding Lab runner: GOOS-conditional cmd.exe vs /bin/sh).
//     Those tests exercise legitimate production behavior, not fake
//     executables — they must be kept working, not rewritten.
//
...
var allowedShellCommandFiles = map[string]string{
	// These tests pass command strings THROUGH the production lab
	// runner/policy (which itself is GOOS-conditional cmd.exe vs
	// /bin/sh) — no test-owned fake executable is created or executed.
	"internal/lab/repair_test.go": "command strings executed by the production lab runner",
	"internal/lab/lab_test.go":    "command strings evaluated by the production lab policy",
}
