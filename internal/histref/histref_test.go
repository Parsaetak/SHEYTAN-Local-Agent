...
// yields ONE clipped excerpt, never an unbounded dump.
func TestResolveBudgetBounded(t *testing.T) {
	store := newCrossModeStore(t)

	blocks := Resolve(store, []Ref{{SessionID: findAgent(t, store)}}, "orders index", 120)
	...
	if tokens > 120 {
		t.Fatalf("block exceeded the budget: %d > 120 tokens", tokens)
	}
}
...
func TestResolveRangesPinsRetrievalScope(t *testing.T) {
	...
	blocks := Resolve(store,
		...
		"orders table database index everything",
		1800,
	)
